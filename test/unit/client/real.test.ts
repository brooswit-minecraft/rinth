...
describe("createRealTransport", () => {
  ...
  describe("openSocket", () => {
    ...
    test("createVersion rejects with a CliError mapped from a 400 response", async () => {
      ...
      expect(caught).toBeInstanceOf(CliError);
      expect((caught as CliError).exitCode).toBe(ExitCode.ApiError);
      // KAN-720 made toCliError() prefix the endpoint onto the message and
      // record status/endpoint on the CliError; createVersion passes
      // "POST /v2/version" through call(), so assert the API's own message
      // still survives inside that richer message.
      expect((caught as CliError).message).toContain("HTTP 400 POST /v2/version:");
